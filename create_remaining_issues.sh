...
# Array of issues: titles and bodies in order
TITLES=(
  "Build parser to extract negative items"
  "Create dispute letter PDF generator"
  "Create dashboard UI with dummy data"
  "Build letter preview + download UI"
  "Connect API to real credit data source"
  "Store dispute records in database"
  "Integrate AI assistant for letter suggestions"
  "Allow upload of PDF credit reports"
)

BODIES=(
  "Write parser logic that filters tradelines and returns only negative items (collections, charge-offs, etc)."
  "Implement PDFKit or Puppeteer logic to generate Metro 2–style dispute letters based on selected negative items."
  "Build a React component to display mock credit items and allow users to select which to dispute."
  "Enable users to preview the generated letter and download as PDF from frontend."
  "Replace mock data in fetchReport API with actual SmartCredit/Experian API integration."
  "Create a Dispute model in Prisma to save submitted disputes, their status, and timestamps."
  "Use OpenAI API to assist with generating personalized dispute letter language."
  "Build functionality to extract and parse uploaded PDF credit reports as a fallback."
)
...
for i in "${!TITLES[@]}"; do
  ...
  # Create the issue and capture the URL from terminal output
  ISSUE_URL=$(gh issue create --repo "$REPO" --title "$TITLE" --body "$BODY" | grep -o 'https://github.com/[^ ]*')
